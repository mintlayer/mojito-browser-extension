/**
 * UI-free application constants.
 *
 * SECURITY/BUNDLE NOTE: this module must never import React components (or
 * anything else that drags UI into the bundle). It is imported by Web
 * Workers (BTC/Cipher) via the @Helpers barrel — JSX in the worker graph
 * cannot be compiled by the worker build pipeline. The JSX-bearing parts
 * (`walletTypes` icons) live in AppInfo.js, which re-exports everything
 * from here for the regular app import path.
 */
