...
const knownMnemonic = 'labor plate task sniff blanket rose answer gesture time seek crowd capital'
const knownWIF = 'cQQb97816o6xoKiVtaL8AMnzQsbiAXzmEKEQE9xQSwMhDeQXZGDw'
const knownPubkey = '02c99ecdb433cce49ed3ce5cda347366446610134c78be6419dea6bd51addb7865'
...
test('Keys Generation - auto reference', () => {
  ...
  const generatedAddress = bitcoin.payments.p2pkh({ pubkey: pubKey, network: bitcoin.networks['testnet'] }).address

  const keyPair = ECPair.fromWIF(WIF, bitcoin.networks['testnet'])
  const generatedAddress2 = bitcoin.payments.p2pkh({ pubkey: keyPair.publicKey, network: bitcoin.networks['testnet']}).address
