...
const formatedReceiverAddress = formatAddress(receiverData.BTC_RECEIVING_ADDRESS)

test('Create BTC transaction', async () => {
  test.setTimeout(120000)
   await page.click(
     'li.crypto-item[data-testid="crypto-item"] h5:text("Bitcoin (Test)")',
   )
  await page.waitForTimeout(1000)
  await page.click('button.button-transaction-up')
  await expect(page.locator(':text("Send to:")')).toBeVisible()
  
  await page.fill('input[placeholder="tb1... or 1... or 3..."]', receiverData.BTC_RECEIVING_ADDRESS)
  ...
  await expect(page.getByTestId('popup').getByText('Enter your password')).toBeVisible()
  ...
  await expect(page.locator(`:text("not confirmed")`).nth(0),)
  .toBeVisible()
})
